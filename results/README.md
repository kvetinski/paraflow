...
`results/day07/` contains deterministic verification receipts produced by
replaying the checked-in evidence. A receipt is not a new benchmark result. It
identifies the verified artifact, reports which deterministic layers passed,
and states whether actual engine bytes were supplied.
...
Use `labctl verify` to audit an existing artifact without changing it.
